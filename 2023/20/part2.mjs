...
class Module {
  ...
  static getPressesForRx() {
    const flipFlops = Module.#instances.get('broadcaster').outputs;
    let total = 1;
    for (const flipFlop of flipFlops) {
      let n = '';
      let current = Module.#instances.get(flipFlop);
      while (current) {
        const { outputs } = current;
        const outputModules = Array.from(outputs, (output) =>
          Module.#instances.get(output)
        );
        const hasConjunction = outputModules.some(
          (output) => output instanceof Conjunction
        );
        n = (hasConjunction ? '1' : '0') + n;
        current = outputModules.find((output) => output instanceof FlipFlop);
      }
      total *= parseInt(n, 2);
    }
    return total;
...
console.log(Module.getPressesForRx());
